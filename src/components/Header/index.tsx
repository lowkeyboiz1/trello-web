...
function Header() {
  ...
  return (
    <header className="flex items-center justify-between px-2 h-12 border-b-[0.5px] border-[#333] bg-[#10b981] text-white overflow-x-auto">
      <div className="flex items-center">
        ...
        <Button className="flex items-center gap-2 text-white" variant="bordered" startContent={<Add />}>
          Create
        </Button>
      </div>
      <div className="flex items-center gap-4">
        <Input
          ...
          endContent={<Add size={16} className="rotate-45 text-black" />}
          classNames={{
            inputWrapper: "h-10",
          }}
...
const ExpandButton: React.FC<{ title: string }> = ({ title }) => {
  return (
    <Button className="flex items-center gap-2 font-medium text-white" variant="light" endContent={<ArrowDown2 size={16} />}>
      {title}
    </Button>
  )
}
